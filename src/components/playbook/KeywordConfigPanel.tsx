...
export default function KeywordConfigPanel({
  ...
  // Auto-generate need words on mount if empty
  // Always try LLM generation - the prompt handles cases with or without product defined
  useEffect(() => {
    ...
    if (needWords.length === 0 && !isGeneratingNeedWords && !hasAutoGenerated) {
      // Always try to generate via LLM - the API handles empty product gracefully
      console.log('[KeywordConfigPanel] Generating need words via LLM', product ? `for product: ${product}` : '(generic)')
      setHasAutoGenerated(true)
      onGenerateNeedWords()
    }
  }, [needWords.length, isGeneratingNeedWords, hasAutoGenerated, product, onGenerateNeedWords])
  ...
  useEffect(() => {
    if (indicators.length === 0) {
      const defaultIndicators = [
        ...
        { id: 'no_aguanto', label: 'no aguanto más', selected: false },
        ...
        { id: 'no_se_que_hacer', label: 'no sé qué hacer', selected: false },
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-lg font-semibold text-gray-900">
            {contextType === 'business' ? '🏢 Condiciones del Negocio' :
             contextType === 'personal' ? '👤 Situaciones de Vida' :
             '👤 Situaciones y Condiciones'}
          </h2>
          <Tooltip content={
            contextType === 'business'
              ? 'Tipo de empresa o etapa en la que se encuentra: startup, PYME, en expansión, etc.'
      ...
      <section>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-lg font-semibold text-gray-900">🔍 Palabras de Necesidad</h2>
          <Tooltip content='Palabras que la gente usa cuando busca ayuda: "problemas con", "cómo hacer", "necesito", etc.'>
            <HelpCircle size={16} className="text-gray-400 cursor-help" />
          </Tooltip>
          <span className="ml-auto text-sm text-gray-500">
      ...
      {/* SECTION 3: Señales de Frustración (Opcional) */}
      <section>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-lg font-semibold text-gray-900">😤 Señales de Frustración</h2>
          <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded">Opcional</span>
          <Tooltip content='Palabras que indican urgencia: "estoy harto", "no aguanto", "ayuda urgente". Ayudan a filtrar los casos más urgentes.'>
            <HelpCircle size={16} className="text-gray-400 cursor-help" />
          </Tooltip>
          <span className="ml-auto text-sm text-gray-500">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder='Agregar señal (ej: "estoy harto de")'
      ...
      {/* SECTION 4: Dónde Buscar */}
      <section>
        <div className="flex items-center gap-2 mb-4">
          <h2 className="text-lg font-semibold text-gray-900">🌐 Dónde Buscar</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-orange-50 rounded-xl border border-orange-100">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-medium text-gray-900">Subreddits Específicos</h3>
          ...
          <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-medium text-gray-900">Foros Temáticos</h3>
              <span className="text-xs text-gray-500 ml-auto">
                {forumsCount} foros
              </span>
            </div>
            ...
            <div className="flex flex-col gap-1 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-blue-700">
                  <strong>{selectedContexts.length}</strong> situaciones × <strong>{selectedNeeds.length}</strong> necesidades × <strong>{totalSources}</strong> fuentes
                  {selectedIndicators.length > 0 && (
                    <span className="text-blue-500"> × <strong>{selectedIndicators.length + 1}</strong> variantes</span>
                  )}
                </span>
                <span className="text-blue-600 font-semibold">
                  = <strong>{selectedContexts.length * selectedNeeds.length * totalSources * (selectedIndicators.length > 0 ? (selectedIndicators.length + 1) : 1)}</strong> búsquedas
                </span>
              </div>
              {selectedIndicators.length > 0 && (
                <p className="text-xs text-blue-500">
                  {selectedIndicators.length + 1} variantes = sin señal + {selectedIndicators.length} señales seleccionadas
                </p>
              )}
            </div>
          </div>
